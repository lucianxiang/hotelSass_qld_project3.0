...
export const getComponentsList = (routes, targetName) => {
  if (routes && routes.length) {
    for (let item of routes) {
      if (item.name === targetName) {
        item.expand = true;
      } else if (item.children && item.children.length) {
        getComponentsList(item.children, targetName);
      }
    }
  }
}
...
export const FormattedData = (data) => {
  ...
  Array.prototype.GroupBy = function (fn_obj_val) {
    ...
    for (var key = 0; key < this.length; key++) {
      ...
      var tArry = nArry.FirstOrDefault(function (n) {
        return fn_obj_val(n[0]) == tVal;
      });
      if (tArry == null) {
        tArry = new Array();
        tArry.push(this[key]);
  ...
  return nList.GroupBy(function (u) {
    return u.Tung;
  }).Select(function (u) {
    var LayerArry = u.GroupBy(function (n) {
      return n.Layer;
    });
    return {
      Tung: u[0].Tung,
...
export const getDateValue = (v, d) => {
  var day = new Date();
  switch (v) {
    case '本周':
      var num = day.getDay() - 1;
      day.setDate(day.getDate() - num); //本周第一天
      var from = day.format("yyyy-MM-dd") + ' 00:00:00';
      day.setDate(day.getDate() + 6);//本周最后一天
      var to = day.format("yyyy-MM-dd") + ' 23:59:59';
      return {from, to}
      break;
    case '下周':
      var num = day.getDay() - 1;
      day.setDate(day.getDate() - num + 7); //下周第一天
      var from = day.format("yyyy-MM-dd") + ' 00:00:00';
      day.setDate(day.getDate() + 6);//下周最后一天
      var to = day.format("yyyy-MM-dd") + ' 23:59:59';
      return {from, to}
      break;
    case '上周':
      var num = day.getDay() - 1;
      day.setDate(day.getDate() - num - 7); //上周第一天
      var from = day.format("yyyy-MM-dd") + ' 00:00:00';
      day.setDate(day.getDate() + 6);//上周最后一天
      var to = day.format("yyyy-MM-dd") + ' 23:59:59';
      return {from, to}
      break;
    case '今天':
      var from = day.format("yyyy-MM-dd") + ' 00:00:00';
      return {from, to: day.format("yyyy-MM-dd") + ' 23:59:59'}
      break;
    case '明天':
...
export const getToday = () => {
  var day = new Date();
  var str = day.format("yyyy-MM-dd hh:mm:ss");
  return str
}
  ...
  for (var i = 0; i < len; i++) {
    ...
    if (!temp[val]) {
      ...
      for (var j = 0; j < r.length; j++) {
        r[j].levelList = val.levelList.concat(r[j].levelList)
      }
    }
  }
  return r;
...
//http://saas.1000da.com.cn
export const getNewStr = 'http://hly.1000da.com.cn';//   /api
export const getNewStr1 = 'http://saas.1000da.com.cn';//   /api
